...
def threshold_measurement_dicho_model(do_experiment, V0 = 0.*mV):
    ...
    print I_peaks
#    idx_th = where(array(I_peaks)<=-1000.1e-12)[0][0]  #it finds the peak axonal current, units are A
#    print idx_th, I_peaks[idx_th]
#    v_threshold = Vc_peaks[idx_th-1] 
#    print 'Rough threshold:', v_threshold
    
    idx_th = where(array(I_peaks)<=-150000.1e-12)[0]  #it finds the peak axonal current, units are A
    if size(idx_th) == 0:
        ...
        if do_experiment:
            ...
            while True:
                ...
                if n_it > 20:
                    ...
                    break
                if i_max >= 0.5*nA and abs(ampli_current - ampli_min) <= 0.2*mV and spike == False:
                    print 'stop'
                    break
                if i_max <= 0.5*nA:
