...
def threshold_measurement(do_experiment, V0 = 0.*mV):
    ...
    print I_peaks
    idx_th = where(array(I_peaks)<=-200.1e-12)[0][0]  #it finds the peak axonal current, units are A !!! change this for CGC
    #idx_th = where(array(I_peaks)<=-200.1e-12)[0][0]
    ...
    if do_experiment:
        ...
        #os.mkdir(path)
        # Saving current script
        #shutil.copy('threshold_measurement.py', path)
